...
if (boxTourDetail) {
    ...
    const drawBoxDetail = () => {
        ...
        const priceChildren = parseInt(
            inputStockChildren.getAttribute("price"),
        );
    ...
    // Bước 4
    const buttonAddToCart = boxTourDetail.querySelector(
        ".inner-button-add-cart",
    );
    buttonAddToCart.addEventListener("click", () => {
        ...
        const quantityBaby = parseInt(inputStockBaby.value);
        const locationFrom =
            boxTourDetail.querySelector("[location-from]").value;
        ...
        if (quantityAdult > 0 || quantityChildren > 0 || quantityBaby > 0) {
            ...
            const indexItemExist = cart.findIndex(
